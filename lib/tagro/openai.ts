...
export async function runOpenAIJson({
  ...
  try {
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: process.env.TAGRO_OPENAI_MODEL || process.env.VEYRA_OPENAI_MODEL || 'gpt-4o-mini',
    ...
    return {
      ...
      model: data?.model ?? process.env.TAGRO_OPENAI_MODEL ?? process.env.VEYRA_OPENAI_MODEL ?? 'gpt-4o-mini',
