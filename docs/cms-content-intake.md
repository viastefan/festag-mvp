...
the sync bus (`lib/sync/bus.ts`) + notifications, and Veyra.
...
one (or Veyra pre-generates the structure from the project brief), then tweaks.
...
1. Dev opens the project → **Inhalte** → picks a template (or Veyra generates
...
5. Veyra can **summarize** the intake, flag missing required fields, and turn
...
- Section status flips drive Veyra status points (same pattern as task updates).
...
3. **Notifications + status** — sync-bus events, section status, Veyra points.
4. **Export** — Markdown/CSV/copy + image zip.
5. **Templates library** — per website type + Veyra auto-generation.
...
work types / Execution Panel, Veyra task-flow.
