...
import ThemeToggle from '@/components/ThemeToggle'
  ...
  aiBubble:'#1C1F1C', userBubble:'#2A2D2A',
}
const LIGHT = {
  ...
  aiBubble:'#ECEEED', userBubble:'#323635',
}

// Step 'ai' ist immer zuerst, dann die Dateneingabe
type Step = 'ai'|'name'|'company'|'phone'|'verify'|'notifs'|'photo'
const DATA_STEPS: Step[] = ['name','company','phone','verify','notifs','photo']
const ALL_STEPS: Step[] = ['ai',...DATA_STEPS]

type Msg = { role:'ai'|'user'; text:string }

const AI_SYSTEM = `Du bist Tagro, das AI-System von Festag. Du führst ein kurzes, freundliches Aufnahmegespräch mit einem neuen Client.

REGELN:
- Maximal 2 Sätze pro Antwort
- Kein Emoji, keine Floskeln
- Klingt klar und menschlich, nicht wie ein Bot
- Stelle gezielte Folgefragen um zu verstehen: Projektziel, Branche, Timeline, Budget-Rahmen
- Nach 3-4 Antworten des Users: Fasse kurz zusammen und beende mit genau diesem JSON auf einer eigenen Zeile: {"ready":true,"summary":"..."}
- Das JSON muss reines JSON sein, kein Markdown
- Sprache: Deutsch`
...
export default function OnboardingPage() {
  ...
  const [msgs, setMsgs] = useState<Msg[]>([
    { role:'ai', text:'Was bringt dich zu Festag? Erzähl mir von deinem Projekt — auch wenn es noch eine grobe Idee ist.' }
  ])
    ...
    try {
      ...
      if (jsonMatch) {
        try {
          ...
          setMsgs(m => [...m, { role:'ai', text: displayText || 'Perfekt — ich habe alles notiert. Jetzt richten wir noch deinen Account ein.' }])
  ...
  async function saveCompany(){
    setLoading(true)
    await supabase.from('profiles').update({company_name:company,company_description:compDesc||aiSummary,company_size:compSize,website,onboarding_step:2}).eq('id',userId)
    ...
    <>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .msg-in{animation:msgIn .25s cubic-bezier(.16,1,.3,1) both;}
        input::placeholder,textarea::placeholder{color:${T.textMut};opacity:1;}
        body{background:${T.bg}!important;}
        select{-webkit-appearance:none;appearance:none;}
        .skip-btn{width:100%;padding:14px;text-align:center;color:${T.textMut};font-size:14px;font-weight:600;cursor:pointer;background:none;border:none;font-family:inherit;margin-top:8px;}
        .ai-input:focus{outline:none;}
      `}</style>

      <div style={{minHeight:'100dvh',background:T.bg,fontFamily:"'Aeonik',sans-serif",WebkitFontSmoothing:'antialiased',display:'flex',flexDirection:'column'}}>

        {/* Progress bar */}
        <div style={{position:'fixed',top:0,left:0,right:0,height:2,background:T.border,zIndex:50}}>
          <div style={{height:'100%',width:`${((stepIdx+1)/ALL_STEPS.length)*100}%`,background:T.accent,transition:'width .4s cubic-bezier(.16,1,.3,1)'}}/>
        </div>
        ...
        {step === 'ai' && (
          <div style={{flex:1,display:'flex',flexDirection:'column',maxWidth:560,margin:'0 auto',width:'100%',
            ...
            <div style={{padding:'0 24px'}}>
              <button onClick={back} style={{background:'transparent',border:'none',color:T.textMut,fontSize:13,cursor:'pointer',padding:0,display:'flex',alignItems:'center',gap:6,marginBottom:28,fontFamily:'inherit',fontWeight:600}}>
            ...
            <div style={{padding:'0 24px',marginBottom:24}}>
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10}}>
                <div style={{width:32,height:32,borderRadius:10,background:T.card,border:`1px solid ${T.border}`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={T.accentSec} strokeWidth="2" strokeLinecap="round"><path d="M12 3v4M12 17v4M3 12h4M17 12h4"/></svg>
                </div>
                <div>
                  <p style={{fontSize:12,fontWeight:700,color:T.accentSec,letterSpacing:'.1em'}}>TAGRO AI</p>
                </div>
              </div>
              <h1 style={{fontSize:28,fontWeight:700,color:T.text,letterSpacing:'-.6px',lineHeight:1.2,marginBottom:6}}>
                Erzähl uns von<br/>deinem Projekt.
              </h1>
              <p style={{fontSize:15,color:T.textSec,lineHeight:1.5,fontWeight:500}}>
                Ein kurzes Gespräch — damit wir die richtigen Menschen für dich finden.
              </p>
            </div>

            {/* Chat messages */}
            <div ref={chatRef} style={{flex:1,overflowY:'auto',padding:'0 24px',display:'flex',flexDirection:'column',gap:12,minHeight:0}}>
              {msgs.map((m,i)=>(
                <div key={i} className="msg-in" style={{display:'flex',justifyContent:m.role==='user'?'flex-end':'flex-start'}}>
                  <div style={{
                    ...
                    background:m.role==='ai'?T.aiBubble:T.btnPrim,
                    color:m.role==='user'?T.btnPrimText:T.text,
              ...
              {aiLoading && (
                <div style={{display:'flex',justifyContent:'flex-start'}}>
                  <div style={{padding:'12px 18px',borderRadius:'18px 18px 18px 4px',background:T.aiBubble,display:'flex',gap:5,alignItems:'center'}}>
                    {[0,1,2].map(i=>(
                      <span key={i} style={{width:6,height:6,borderRadius:'50%',background:T.textMut,animation:`blink 1.2s ${i*0.2}s infinite`}}/>
                    ))}
                  </div>
                </div>
              )}
            ...
            <div style={{padding:'16px 24px',paddingBottom:`calc(env(safe-area-inset-bottom) + 24px)`}}>
              {!aiDone ? (
                <div style={{display:'flex',gap:10,alignItems:'flex-end',background:T.card,border:`1.5px solid ${T.border}`,borderRadius:16,padding:'12px 16px'}}>
                  <textarea
                    className="ai-input"
                    value={aiInput}
                    ...
                    rows={1}
                    style={{flex:1,background:'transparent',border:'none',fontSize:15,color:T.text,fontFamily:'inherit',fontWeight:500,resize:'none',lineHeight:1.5,maxHeight:120,overflowY:'auto'}}
                  />
                  <button onClick={sendAiMsg} disabled={!aiInput.trim()||aiLoading}
                    style={{width:36,height:36,borderRadius:10,background:aiInput.trim()?T.btnPrim:'transparent',border:`1px solid ${aiInput.trim()?'transparent':T.border}`,display:'flex',alignItems:'center',justifyContent:'center',cursor:aiInput.trim()?'pointer':'default',transition:'all .15s',flexShrink:0}}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={aiInput.trim()?T.btnPrimText:T.textMut} strokeWidth="2.5" strokeLinecap="round"><path d="M22 2L11 13"/><path d="M22 2L15 22 11 13 2 9l20-7z"/></svg>
                  </button>
                </div>
              ) : (
                <div className="ob">
                  <div style={{padding:'14px 18px',background:T.card,border:`1px solid ${T.border}`,borderRadius:14,marginBottom:12}}>
                    <p style={{fontSize:12,fontWeight:700,color:T.accentSec,letterSpacing:'.08em',marginBottom:4}}>TAGRO ZUSAMMENFASSUNG</p>
                    <p style={{fontSize:14,color:T.textSec,lineHeight:1.5}}>{aiSummary}</p>
                  </div>
                  <button onClick={proceedFromAi} style={{width:'100%',padding:'16px',background:T.btnPrim,color:T.btnPrimText,fontSize:16,fontWeight:700,borderRadius:14,border:'none',cursor:'pointer',fontFamily:'inherit',letterSpacing:'-.1px',boxShadow:`0 2px 16px ${T.glow}`}}>
                    Account einrichten →
                  </button>
                </div>
              )}
              ...
              {/* Skip AI */}
              {!aiDone && (
                <button onClick={()=>setStep('name')} style={{width:'100%',marginTop:10,padding:'12px',textAlign:'center',color:T.textMut,fontSize:13,fontWeight:600,cursor:'pointer',background:'none',border:'none',fontFamily:'inherit'}}>
                  Überspringen
                </button>
              )}
            </div>
        ...
        {step !== 'ai' && (
          <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',
            ...
            <div style={{width:'100%',maxWidth:440}} className="ob">
              ...
              {/* Back */}
              <button onClick={back} style={{background:'transparent',border:'none',color:T.textMut,fontSize:13,cursor:'pointer',padding:0,display:'flex',alignItems:'center',gap:6,marginBottom:36,fontFamily:'inherit',fontWeight:600}}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M15 18l-6-6 6-6"/></svg>
                Zurück
              </button>
              ...
              {step==='name' && (<>
                ...
                <p style={{fontSize:16,color:T.textSec,marginBottom:32,lineHeight:1.55,fontWeight:500}}>Damit wir dich richtig ansprechen können.</p>
              ...
              {step==='company' && (<>
                ...
                <p style={{fontSize:16,color:T.textSec,marginBottom:32,lineHeight:1.55,fontWeight:500}}>Hilft der AI, dein Projekt zu verstehen.</p>
                <Inp label="Firmenname (Optional)" value={company} onChange={setCompany} placeholder="Musterfirma GmbH" autoFocus T={T}/>
                <div style={{marginBottom:13}}>
                  <label style={{display:'block',fontSize:11,fontWeight:700,letterSpacing:'.1em',color:T.textMut,textTransform:'uppercase',marginBottom:7}}>BESCHREIBUNG</label>
                  <textarea value={compDesc||aiSummary} onChange={e=>setCompDesc(e.target.value)} placeholder="Was macht dein Unternehmen?"
                    style={{width:'100%',minHeight:80,padding:'14px 16px',background:T.inp,border:`1.5px solid ${T.inpBorder}`,borderRadius:13,fontSize:15,color:T.text,outline:'none',fontFamily:'inherit',fontWeight:500,resize:'vertical'}}/>
                </div>
                <div style={{marginBottom:13}}>
                  <label style={{display:'block',fontSize:11,fontWeight:700,letterSpacing:'.1em',color:T.textMut,textTransform:'uppercase',marginBottom:7}}>GRÖSSE (OPTIONAL)</label>
                  <select value={compSize} onChange={e=>setCompSize(e.target.value)}
                    style={{width:'100%',padding:'14px 16px',background:T.inp,border:`1.5px solid ${T.inpBorder}`,borderRadius:13,fontSize:16,color:compSize?T.text:T.textMut,outline:'none',fontFamily:'inherit',fontWeight:500,cursor:'pointer'}}>
              ...
              {step==='phone' && (<>
                ...
                <p style={{fontSize:16,color:T.textSec,marginBottom:32,lineHeight:1.55,fontWeight:500}}>Wir senden dir einen Verifizierungscode.</p>
              ...
              {step==='verify' && (<>
                ...
                <p style={{fontSize:16,color:T.textSec,marginBottom:32,lineHeight:1.55,fontWeight:500}}>Gesendet an <span style={{color:T.text,fontWeight:700}}>+49 {phone}</span></p>
                ...
                <p style={{textAlign:'center',fontSize:13,color:T.textMut,marginTop:18}}>
                  Nichts erhalten?{' '}
                  {cd>0
              ...
              {step==='notifs' && (<>
                ...
                <p style={{fontSize:16,color:T.textSec,marginBottom:28,lineHeight:1.55,fontWeight:500}}>Wähle was für dich wichtig ist.</p>
                <NRow title="AI Updates & Tagesberichte" desc="Wenn Tagro Berichte erstellt" on={nAI} setOn={setNAI} T={T}/>
                <NRow title="Developer-Aktivität" desc="Tasks erledigt oder aktiv" on={nDev} setOn={setNDev} T={T}/>
                ...
                <div style={{marginTop:20}}><PBtn label="Weiter" onClick={saveNotifs} loading={loading} T={T}/></div>
              ...
              {step==='photo' && (<>
                ...
                <p style={{fontSize:16,color:T.textSec,marginBottom:32,lineHeight:1.55,fontWeight:500}}>Optional — jederzeit in den Einstellungen änderbar.</p>
                <label htmlFor="photo-up" style={{display:'block',cursor:'pointer'}}>
                  <div style={{width:88,height:88,borderRadius:28,margin:'0 auto 20px',background:photoUrl?'transparent':T.card,border:`2px dashed ${photoUrl?T.accent:T.border}`,display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',transition:'all .2s',boxShadow:photoUrl?`0 0 0 4px ${T.glow}`:'none'}}>
                    {photoUrl
                      ? <img src={photoUrl} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>
                      : <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={T.textMut} strokeWidth="1.5"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg>}
                  </div>
                </label>
...
function NRow({title,desc,on,setOn,T}:{title:string;desc:string;on:boolean;setOn:(v:boolean)=>void;T:typeof DARK}) {
  return (
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:16,padding:'15px 16px',background:T.card,borderRadius:13,border:`1px solid ${T.border}`,marginBottom:9}}>
      <div>
        <p style={{fontSize:14,fontWeight:700,color:T.text,marginBottom:2}}>{title}</p>
        <p style={{fontSize:12,color:T.textSec,lineHeight:1.4}}>{desc}</p>
      </div>
      <div onClick={()=>setOn(!on)} style={{width:42,height:24,borderRadius:12,background:on?T.btnPrim:'rgba(255,255,255,0.1)',position:'relative',cursor:'pointer',transition:'all .2s',flexShrink:0}}>
        <div style={{position:'absolute',top:2,left:on?19:2,width:20,height:20,borderRadius:10,background:on?T.bg:'#666',transition:'left .2s cubic-bezier(.16,1,.3,1)',boxShadow:'0 1px 4px rgba(0,0,0,.3)'}}/>
      </div>
    </div>
  )
}
