  ...
  const submit = async () => {
    ...
    if (mode === 'register') {
      ...
      if (portal === 'developer') {
        ...
        } else { setError('Kein Developer-Zugang für dieses Konto.'); setLoading(false); return }
      } else {
      ...
      <div style={{ width: '100%', maxWidth: 440, animation: 'fadeUp 0.4s cubic-bezier(0.16, 1, 0.3, 1) both' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[
            ...
            { key: 'developer', label: 'Developer & Admin', desc: 'Für Teams', badge: 'Intern' },
          ].map(p => (
...
const inp: React.CSSProperties = { width: '100%', padding: '12px 14px', background: '#FFFFFF', border: '1px solid var(--border)', borderRadius: 'var(--r-sm)', fontSize: 15, outline: 'none', color: 'var(--text)', boxSizing: 'border-box' as const, transition: 'all 0.15s', minHeight: 44 }
