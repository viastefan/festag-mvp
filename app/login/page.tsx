  ...
  return (
    <div style={{ position:'relative', flex:1, overflow:'hidden', minWidth:0, minHeight:'100%' }}>
      ...
      <div style={{
        ...
        background:'linear-gradient(to right, transparent, rgba(255,255,255,0.08) 70%, rgba(255,255,255,0.45))',
        pointerEvents:'none',
      }}/>
      ...
      {portal === 'select' && (
        <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'0 52px', textAlign:'center', zIndex:5 }}>
          ...
          `}</style>
          <p className="tl1" style={{ fontFamily:"'Aeonik',sans-serif", fontWeight:400, fontSize:11, letterSpacing:'0.22em', color:'rgba(255,255,255,0.5)', textTransform:'uppercase', marginBottom:20 }}>
            Festag — AI-native Softwareproduktion
          </p>
          <h2 className="tl2" style={{ fontFamily:"'Aeonik',sans-serif", fontWeight:700, fontSize:38, color:'#fff', lineHeight:1.15, letterSpacing:'-0.5px', marginBottom:18, maxWidth:460, textShadow:'0 2px 24px rgba(0,0,0,0.4)' }}>
            Kein lahmes Onboarding. Kein Risiko bei Abnahme.
          </h2>
          <p className="tl3" style={{ fontFamily:"'Aeonik',sans-serif", fontWeight:500, fontSize:19, color:'rgba(255,255,255,0.75)', letterSpacing:'-0.1px' }}>
            Festag baut. Du trinkst Kaffee.
          </p>
        </div>
      )}
      {portal !== 'select' && (
        <div style={{ position:'absolute', bottom:48, left:36, right:36, zIndex:5 }}>
          <h2 style={{ fontFamily:"'Aeonik',sans-serif", fontWeight:700, fontSize:30, color:'#fff', lineHeight:1.2, letterSpacing:'-0.4px', marginBottom:10, textShadow:'0 2px 20px rgba(0,0,0,0.4)' }}>
            {portal === 'client' ? 'Software, verständlich gemacht.' : 'Team Execution System.'}
          </h2>
          <p style={{ fontFamily:"'Aeonik',sans-serif", fontWeight:400, fontSize:14, color:'rgba(255,255,255,0.65)', lineHeight:1.6 }}>
      ...
      <p style={{ position:'absolute', bottom:14, left:0, right:0, textAlign:'center', fontSize:10, letterSpacing:'.1em', color:'rgba(255,255,255,0.25)', fontFamily:"'Aeonik',sans-serif", fontWeight:400 }}>
  ...
  if (portal === 'select') return (
    <div style={{ minHeight:'100vh', display:'flex', background:'#050e1c', fontFamily:"'Aeonik',sans-serif" }}>
      <style>{`
        ...
        .p-btn{background:#fff;border:1.5px solid #EEF2F7;border-radius:16px;padding:18px 20px;cursor:pointer;display:flex;align-items:center;justify-content:space-between;gap:12px;font-family:'Aeonik',sans-serif;transition:all .15s;text-align:left;width:100%;}
        .p-btn:hover{border-color:#CBD5E1;background:#FAFBFD;box-shadow:0 4px 20px rgba(0,0,0,.07);transform:translateY(-1px);}
      ...
      <div style={{ display:'flex', width:'100%', minHeight:'100vh' }}>
        ...
        <div className="sel-cards">
          <div style={{ marginBottom:32 }}>
            <img src="/brand/logo.svg" alt="festag" style={{ height:22, display:'block', marginBottom:32 }} />
            <p style={{ fontSize:11, fontWeight:700, letterSpacing:'.12em', color:'#007AFF', marginBottom:10 }}>WÄHLE DEINEN ZUGANG</p>
            <h1 style={{ fontSize:25, fontWeight:700, color:'#0F172A', letterSpacing:'-0.4px', marginBottom:4 }}>Willkommen zurück.</h1>
            <p style={{ fontSize:14, color:'#94A3B8', margin:0 }}>{greeting.label} — {greeting.sub}</p>
          </div>
              ...
              <div>
                <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
                  <div style={{ width:28, height:28, borderRadius:8, background:'#EFF6FF', display:'flex', alignItems:'center', justifyContent:'center' }}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#007AFF" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                  </div>
                  <p style={{ fontSize:15, fontWeight:700, color:'#0F172A', margin:0 }}>Client</p>
                </div>
                <p style={{ fontSize:13, color:'#94A3B8', margin:'0 0 0 36px' }}>Für Auftraggeber & Kunden</p>
              </div>
              ...
              <div>
                <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
                  <div style={{ width:28, height:28, borderRadius:8, background:'#ECFDF5', display:'flex', alignItems:'center', justifyContent:'center' }}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/></svg>
                  </div>
                  <p style={{ fontSize:15, fontWeight:700, color:'#0F172A', margin:0 }}>Developer</p>
                </div>
                <p style={{ fontSize:13, color:'#94A3B8', margin:'0 0 0 36px' }}>Interner Zugang · Nutzername & PIN</p>
              </div>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#CBD5E1" strokeWidth="2" strokeLinecap="round"><path d="M9 6l6 6-6 6"/></svg>
            </button>
          </div>
          <p style={{ textAlign:'center', fontSize:11, color:'#E2E8F0', marginTop:36, letterSpacing:'.07em' }}>AI PLANT · MENSCHEN BAUEN · SYSTEM VERBINDET</p>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', background:'#fff', fontFamily:"'Aeonik',sans-serif" }}>
      <style>{`
        ...
        .login-right{width:100%;max-width:480px;padding:48px 40px;display:flex;flex-direction:column;flex-shrink:0;}
        @media(max-width:768px){
          .login-left{display:none !important;}
          .login-right{max-width:100%;padding:32px 20px;padding-top:calc(32px + env(safe-area-inset-top));}
        }
      `}</style>
