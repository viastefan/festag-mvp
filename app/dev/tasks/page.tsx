...
export default function DevTasksPage() {
  ...
  useEffect(() => {
    ...
    supabase.from('tasks').select('*, projects(title)').eq('assigned_to', info.user_id).order('created_at', { ascending: false }).then(({ data }) => {
    ...
    <div className="animate-fade-up">
      ...
      {loading ? (
        ...
        <div style={{ background: 'var(--surface)', border: '1px dashed var(--border)', borderRadius: 'var(--r-lg)', padding: '50px 20px', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>Nimm einen Job an um Tasks zu erhalten.</p>
        </div>
      ) : (
        <div className="grid-cols-2-mobile-1" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
          {cols.map(c => {
            ...
            return (
              <div key={c.status}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {colTasks.map(t => (
                    <div key={t.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: '12px 14px', transition: 'border-color 0.2s' }}>
                      ...
                      <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                        {cols.filter(x => x.status !== c.status).map(x => (
                          <button key={x.status} onClick={() => updateStatus(t.id, x.status)} className="tap-scale" style={{ padding: '4px 8px', fontSize: 10, border: '1px solid var(--border)', background: 'var(--bg)', borderRadius: 6, cursor: 'pointer', color: 'var(--text-secondary)', fontFamily: 'inherit' }}>→ {x.label}</button>
                        ))}
                      </div>
                    </div>
                  ))}
                  {colTasks.length === 0 && <div style={{ border: '1px dashed var(--border)', borderRadius: 'var(--r)', padding: 16, textAlign: 'center', fontSize: 11, color: 'var(--text-muted)' }}>Keine Tasks</div>}
                </div>
              </div>
            )
          })}
