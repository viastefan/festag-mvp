...
export default function AIHubPage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      ...
      supabase.from('projects').select('title,status').limit(5).then(({ data: p }) => setProjects(p ?? []))
    })
  }, [])
  ...
  async function send(text?: string) {
    ...
    try {
      ...
      setMsgs(m => [...m, { role:'ai', text:'STATUS: System-Fehler.\nANALYSE: Verbindung unterbrochen.\nAKTION: Fehler geloggt.\nNÄCHSTE SCHRITTE: Erneut versuchen.', time: new Date().toLocaleTimeString('de', { hour:'2-digit', minute:'2-digit' }) }])
    }
    setLoading(false)
  }

  const KEYS: Record<string,string> = { 'STATUS':'STATUS', 'ANALYSE':'ANALYSE', 'AKTION':'AKTION', 'NÄCHSTE SCHRITTE':'NÄCHSTE SCHRITTE' }

  function parseLines(text: string) {
    return text.split('\n').filter(Boolean).map(line => {
      const i = line.indexOf(':')
      if (i > 0 && i < 22 && KEYS[line.slice(0,i).trim()]) {
        return { key: line.slice(0,i).trim(), val: line.slice(i+1).trim() }
      }
      return { key: '', val: line }
    })
  }
    ...
    <div className="animate-fade-up" style={{ maxWidth: 800, margin: '0 auto', display: 'flex', flexDirection: 'column', height: 'calc(100vh - 120px)' }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 14, color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          Production Engine · {projects.length} Projekt{projects.length !== 1 ? 'e' : ''}
        </p>
      </div>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              {m.role==='ai' ? (
                <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'var(--r)', overflow:'hidden' }}>
                  {parseLines(m.text).map((line, j) => (
                    <div key={j} style={{ display:'flex', borderBottom: j<parseLines(m.text).length-1?'1px solid var(--border)':'none' }}>
                      {line.key && (
                        <div style={{ padding:'9px 14px', minWidth:140, background:'var(--bg)', borderRight:'1px solid var(--border)', display:'flex', alignItems:'center' }}>
                          <span style={{ fontSize:10, fontWeight:700, color:'var(--text-muted)', letterSpacing:'0.08em' }}>{line.key}</span>
                        </div>
                      )}
                      <div style={{ padding:'9px 14px', flex:1 }}>
                        <p style={{ fontSize:13, color:'var(--text)', margin:0, lineHeight:1.55 }}>{line.val}</p>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <div style={{ paddingTop:10, display:'flex', gap:6, overflowX:'auto', paddingBottom:6 }}>
        {QUICK.map(q => (
          <button key={q} onClick={() => send(q)} className="tap-scale" style={{ padding:'6px 12px', borderRadius:20, border:'1px solid var(--border)', background:'var(--surface)', fontSize:12, color:'var(--text-secondary)', cursor:'pointer', whiteSpace:'nowrap', flexShrink:0, fontFamily:'inherit' }}>{q}</button>
        ))}
      </div>
      ...
      <div style={{ display:'flex', gap:8 }}>
        ...
        <button
          ...
          className="tap-scale"
          style={{ width:44, height:44, borderRadius:'var(--r)', border:'none', flexShrink:0, background: input.trim()&&!loading ? 'var(--text)' : 'var(--surface-2)', color: input.trim() ? '#fff' : 'var(--text-muted)', cursor: input.trim() ? 'pointer' : 'default', display:'flex', alignItems:'center', justifyContent:'center' }}
        >
          {loading
