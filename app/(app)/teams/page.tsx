...
const ICONS = {
  ...
  corporate: <><rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/></>,
...
const SCENARIOS = [
  ...
  },
  {
    ...
    desc: 'Die Agentur verwaltet 50 Teams für 50 Kunden. Jeder Client = ein isolierter Team-Context. Kunde A sieht niemals Projekte von Kunde B.',
  ...
  return (
    <div className="page-content animate-fade-up" style={{ maxWidth: 1100 }}>
      <style>{`
        ...
        @keyframes inv-pop { from { opacity:0; transform:scale(.96) translateY(10px); } to { opacity:1; transform:none; } }
        ...
        .inv-backdrop { position:absolute; inset:0; background:rgba(0,0,0,0.55); backdrop-filter:blur(8px); -webkit-backdrop-filter:blur(8px); }
        .inv-panel { position:relative; width:100%; max-width:460px; background:var(--surface); border:1px solid var(--border); border-radius:20px; box-shadow:0 32px 80px rgba(0,0,0,0.28); overflow:hidden; animation:inv-pop .22s cubic-bezier(.16,1,.3,1) both; }
      `}</style>
            ...
            <div style={{ padding:'22px 24px 24px' }}>
              {invSent ? (
                <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:14, padding:'24px 0', textAlign:'center' }}>
                  <div style={{ width:52, height:52, borderRadius:'50%', background:'rgba(52,199,89,.12)', border:'1px solid rgba(52,199,89,.25)', display:'flex', alignItems:'center', justifyContent:'center' }}>
          ...
          {SCENARIOS.map((sc, idx) => {
            ...
            return (
              <div key={sc.id} className={`sc-card${active?' active':''}`}
                ...
                style={{ background:'var(--surface)', border:`1.5px solid ${active?'var(--text)':'var(--border)'}`, borderRadius:18, padding:'20px 20px 18px', display:'flex', flexDirection:'column', gap:0, position:'relative', animation:`sc-in .3s ${idx*0.06}s both` }}>
                ...
                <div style={{ marginBottom:16 }}>
                  {sc.visibility.map(v => (
                    <div key={v.label} className="vis-row">
                      <div style={{ width:16, height:16, borderRadius:'50%', background:v.granted?'rgba(52,199,89,.12)':'rgba(239,68,68,.1)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                        {v.granted
                          ? <Ico path={ICONS.check} size={8} sw={2.8} color="var(--green)" />
                          : <Ico path={<><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></>} size={7} sw={2.5} color="#EF4444" />
                        }
                      </div>
                      <span style={{ fontSize:11.5, color:v.granted?'var(--text-secondary)':'var(--text-muted)', flex:1, lineHeight:1.3 }}>{v.label}</span>
                      {!v.granted && <Ico path={ICONS.eyeOff} size={11} sw={1.6} color="var(--text-muted)" />}
                    </div>
                  ))}
                </div>

                {(sc as any).mailto ? (
                  <a href={(sc as any).mailto} onClick={e => e.stopPropagation()}
                  ...
                  <button onClick={openInvite}
                    style={{ display:'block', width:'100%', padding:'10px 14px', background:active?'var(--btn-prim)':'var(--surface-2)', color:active?'var(--btn-prim-text)':'var(--text)', borderRadius:11, fontSize:12.5, fontWeight:700, textAlign:'center', border:`1px solid ${active?'transparent':'var(--border)'}`, cursor:'pointer', fontFamily:'inherit', transition:'background .15s, color .15s' }}>
                    {sc.cta}
                  </button>
                )}
              </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 340px', gap:12 }} className="grid-cols-2-mobile-1">
        ...
        <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'var(--r-lg)', padding:'18px 20px' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
            ...
            <button onClick={openInvite}
              style={{ display:'flex', alignItems:'center', gap:5, fontSize:12, fontWeight:700, padding:'6px 12px', background:'var(--btn-prim)', color:'var(--btn-prim-text)', border:'none', borderRadius:9, cursor:'pointer', fontFamily:'inherit' }}>
          ...
          {members.length === 0 ? (
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(150px, 1fr))', gap:8 }}>
              {members.map(m => {
                ...
                const roleBg    = m.role==='dev'?'rgba(52,199,89,.12)':m.role==='admin'?'rgba(245,158,11,.12)':'var(--surface-2)'
              ...
              {/* Add slot */}
              <div className="tm-member" onClick={openInvite}
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          ...
          <div style={{ padding:'14px 16px', background:'var(--surface-2)', border:'1px solid var(--border)', borderRadius:'var(--r-lg)', display:'flex', gap:12, alignItems:'flex-start' }}>
            ...
            <p style={{ fontSize:11.5, color:'var(--text-secondary)', margin:0, lineHeight:1.55 }}>
              ...
              Ein User kann beliebig vielen Teams angehören. Agenturen schalten mit einem Klick zwischen Client-Contexts — ohne Account-Wechsel.
            </p>
          </div>
        </div>
      </div>
